...
class RotaryCamWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Brian Rotary CAM v1.5 - Selectable XAZ / YAZ + PyVista")
    ...
    def _build_ui(self):
        ...
        file_layout.addWidget(QtWidgets.QLabel("Rotary alignment"), 1, 0)
        self.linear_axis = QtWidgets.QComboBox()
        self.linear_axis.addItem("X axis — X / A / Z", "X")
        self.linear_axis.addItem("Y axis — Y / A / Z", "Y")
        self.linear_axis.setToolTip(
            "Choose the machine axis running along the rotary workpiece. "
            "The rotary motor remains A in both modes."
        )
        file_layout.addWidget(self.linear_axis, 1, 1, 1, 3)
        self._add_field(file_layout, 2, "stock", "Stock diameter", "", "mm")
        ...
        self._add_field(
            file_layout, 3, "x_step", "Longitudinal step", f"{X_STEP_MM_DEFAULT:.2f}", "mm"
        )
    ...
    def _finishing_box(self):
        ...
        rows = (
            ...
            ("finish_entry_depth", "Entry depth / layer", FINISH_ENTRY_DEPTH_DEFAULT, "mm"),
        ...
        note = QtWidgets.QLabel(
            "Entry layers establish the first finishing corridor.\n"
            "Confirm the cutter taper length reaches full depth."
        )
    ...
    def _parameters(self):
        ...
        return {
            "linear_axis": self.linear_axis.currentData(),
            "stock_diameter": None
            if not stock_text
            ...
            "finish_entry_depth_per_pass": self._positive(
                value("finish_entry_depth"), "Finishing entry depth per layer"
            ),
    ...
    def _result_text(result):
        ...
        return (
            f"Machine output: {result['linear_axis']}/A/Z "
            f"({result['linear_axis']} runs along the rotary workpiece)\n"
            ...
            f"Stepped entry depth/layer: {result['finish_entry_depth_per_pass']:.3f} mm\n"
            ...
            f"Times include {result['linear_axis']}/Z cutting travel only; "
            "inspect in Mach3 and air-cut before machining."
        )
